...
def get_codexplorer_dir(base_dir: str | None = None) -> Path:
    ...
    On first creation, writes a .gitignore to prevent committing profile data.
    """
    base = Path(base_dir) if base_dir else Path.cwd()
    codexplorer_dir = base / "codexplorer"
    first_create = not codexplorer_dir.exists()
    codexplorer_dir.mkdir(parents=True, exist_ok=True)
    gitignore = codexplorer_dir / ".gitignore"
    if first_create and not gitignore.exists():
        gitignore.write_text("# Auto-generated by codexplore — profile data should not be committed\n*\n!.gitignore\n")
    return codexplorer_dir


def generate_run_id() -> str:
    """Generate a timestamped run ID with microsecond precision to avoid collisions."""
